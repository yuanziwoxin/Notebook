...
TiDB 会将执行时间超过 [slow-threshold](https://docs.pingcap.com/zh/tidb/v6.1/tidb-configuration-file#slow-threshold)（默认值为 300 毫秒）的语句输出到 [slow-query-file](https://docs.pingcap.com/zh/tidb/v6.1/tidb-configuration-file#slow-query-file)（默认值："tidb-slow.log"）日志文件中，用于帮助用户定位慢查询语句，分析和解决 SQL 执行的性能问题。

TiDB 默认启用慢查询日志，可以修改配置 [`enable-slow-log`](https://docs.pingcap.com/zh/tidb/v6.1/tidb-configuration-file#enable-slow-log) 来启用或禁用它。
...
- [tidb_slow_log_threshold](https://docs.pingcap.com/zh/tidb/v6.1/system-variables#tidb_slow_log_threshold)：设置慢日志的阈值，执行时间超过阈值的 SQL 语句将被记录到慢日志中。默认值是 300 ms。
- [tidb_query_log_max_len](https://docs.pingcap.com/zh/tidb/v6.1/system-variables#tidb_query_log_max_len)：设置慢日志记录 SQL 语句的最大长度。默认值是 4096 byte。
- [tidb_redact_log](https://docs.pingcap.com/zh/tidb/v6.1/system-variables#tidb_redact_log)：设置慢日志记录 SQL 时是否将用户数据脱敏用 `?` 代替。默认值是 0 ，即关闭该功能。
- [tidb_enable_collect_execution_info](https://docs.pingcap.com/zh/tidb/v6.1/system-variables#tidb_enable_collect_execution_info)：设置是否记录执行计划中各个算子的物理执行信息，默认值是 `1`。该功能对性能的影响约为 3%。开启该项后查看 `Plan` 的示例如下：
...
`Plan` 字段显示的格式和 [`EXPLAIN`](https://docs.pingcap.com/zh/tidb/v6.1/sql-statement-explain) 或者 [`EXPLAIN ANALYZE`](https://docs.pingcap.com/zh/tidb/v6.1/sql-statement-explain-analyze) 大致一致。可以查看 [`EXPLAIN`](https://docs.pingcap.com/zh/tidb/v6.1/sql-statement-explain) 或者 [`EXPLAIN ANALYZE`](https://docs.pingcap.com/zh/tidb/v6.1/sql-statement-explain-analyze) 文档了解更多关于执行计划的信息。

更多详细信息，可以参见 [TiDB 专用系统变量和语法](https://docs.pingcap.com/zh/tidb/v6.1/system-variables)。

## 慢日志内存映射表

用户可通过查询 `INFORMATION_SCHEMA.SLOW_QUERY` 表来查询慢查询日志中的内容，表中列名和慢日志中字段名一一对应，表结构可查看 [`SLOW_QUERY` 表](https://docs.pingcap.com/zh/tidb/v6.1/information-schema-slow-query)中的介绍。
...
TiDB 4.0 中新增了 [`CLUSTER_SLOW_QUERY`](https://docs.pingcap.com/zh/tidb/v6.1/information-schema-slow-query#cluster_slow_query-table) 系统表，用来查询所有 TiDB 节点的慢查询信息，表结构在 `SLOW_QUERY` 的基础上多增加了 `INSTANCE` 列，表示该行慢查询信息来自的 TiDB 节点地址。使用方式和 [`SLOW_QUERY`](https://docs.pingcap.com/zh/tidb/v6.1/information-schema-slow-query) 系统表一样。
